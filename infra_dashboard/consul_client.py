...
class ConsulClient:
    ...
    def get_services(self) -> Optional[Dict]:
        ...
        try:
            response = requests.get(url, headers=self.headers, params={'dc': self.config.CONSUL_DC, 'tag': 'ssl-proxy-backend'})
    ...
    def get_service_instances(self, service_name: str) -> Optional[List[Dict]]:
    ...
    def get_websites_from_consul(self) -> List[Dict]:
        ...
        self.logger.info("%s services founds", len(services))
        if not services:
            return websites
